...
export const useDashboardStore = defineStore('dashboard', {
  ...
  actions: {
    ...
    async getCalendlySchedules(params) {
      return api
        .get('/api/calendly/getData', {
          params,
        })
        .then((response) => response.data)
        .catch((error) => error.response)
    },
    async getGoogleSchedules(params) {
      return api
        .get('/api/google-calendar/getData', {
          params,
        })
        .then((response) => response.data)
        .catch((error) => error.response)
    ...
    logCards(payload) {
      ...
      if (payload === 'Second Call Booked [ Offered ]')
        offerCondition = 'Second Call Booked'
      if (payload === 'Won') offerCondition = 'Sale'

      return this.logs
        ?.filter(
          (log) =>
            log.call_result === condition || log.offer_result === offerCondition
        )
        .map((log) => ({
          id: log.id,
          type: 'draggable',
    ...
    async getUnloggedEvents() {
      ...
      this.getCalendlySchedules(params).then((response) => {
        this.unloggedEvents = response.filter(
          (event) =>
            event.calendar_event.external_id &&
            !eventIds.includes(event.calendar_event.external_id)
        )
      })
    },
  },
