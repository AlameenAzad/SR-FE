  ...
  actions: {
    login(email, password) {
      return api.post('/api/login', {
        ...
        this.calendlyIntegrated = response.data.user.calendly_access_token  ? true : false
        this.calendly = {
    ...
    logout() {
      return api.post('/api/logout').then(async (response) => {
        this.$reset()
    ...
    updateUserInformation(username, userEmail) {
      return api.post('/api/updateInfo', {
        name: username,
        email: userEmail
      }).then((response) => {
    ...
    updateUserPassword(data) {
      return api.post('/api/updatePassword',
        data
      ).then((response) => response).catch((error) => error)
    },
    checkStripeIntegration() {
      return api.get('/api/check-stripe-integration').then((response) => {
    ...
    integrateStripe(data) {
      return api.post('/api/integrate-stripe', data).then((response) => response).catch((error) => error)
    },
    integrateCalendly(code) {
      return api.post('/api/calendly/integrate', {
        code
      }).then((response) => {
        this.calendly = {
    ...
    getCalendlyEventTypes() {
      return api.get('/api/calendly/getCalendlyEventTypes').then((response) => response).catch((error) => error)
    },
    saveCalendlyEventType(data) {
      return api.post('/api/calendly/saveCalendlyEventTypes', data).then((response) => {
    ...
    getCalendlyEventInvitees(uuid) {
      ...
      return api.get('/api/calendly/getCalendlyEventInvitees', {
        params: {
          uuid
        }
      }).then((response) => {
